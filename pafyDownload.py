...
def estado(s):
    boton_directorio.config(state=s)
    boton_descargar.config(state=s)
    boton_audio.config(state=s)

def mycb(total, recvd, ratio, rate, eta):
    global dif
    porcen = (recvd*100/total_size)
    etiqueta_porcentaje.config(text=((int(porcen),'%')))
    progreso.step(porcen-dif)
    dif=porcen
...
def descargando(co, vid):
    ...
    messagebox.showwarning('FIN DE LA DESCARGA', 'Descarga finalizada co éxito')

    estado('normal')
    etiqueta.place(x=317,y=180)
    etiqueta_porcentaje.config(text=' ')
    dif = 0
    total_size = 0
...
directorio_actual()
###############3
entrada = Entry(ventana, font=('Arial',15,'bold'), width=30)
entrada.place(x=196, y=130)

entrada2 = Entry(ventana, font=('Arial', 8), width=60)
...
boton_directorio = Button(ventana, command=directorio,cursor='hand2',width=20, text='CAMBIAR DE DIRECTORIO', bg='pale green')
boton_directorio.place(x=287, y=270)

boton_descargar = Button(ventana,command=lambda:descarga('vid'),cursor='hand2', width=20, text='DESCARGAR VIDEO!', bg='pale green')
boton_descargar.place(x=287, y=310)

Label(ventana, width=12, text='URL de video', bg='navajo white').place(x=316, y=109)

boton_audio = Button(ventana, command=lambda:descarga('vid'),width=20, text='EXTRAER AUDIO ', bg='pale green')
boton_audio.place(x=287, y=350)

etiqueta = Label(ventana, width=12, text='PROGRESO', bg='navajo white')
etiqueta.place(x=317, y=180)

etiqueta_porcentaje = Label(ventana, width=4, bg='navajo white')
etiqueta_porcentaje.place(x=392, y=180)
progreso = progressbar = ttk.Progressbar(ventana)
progreso.place(x=196, y=200, width=335)
###########33
ventana.mainloop()'''
